...
def get_lineouts(
    ...
    for file in files:
        ...
        if xyz:
            xc, yc, zc, linex, liney = extract_line(
                ds,
                field,
                axis,
                Nmax,
...
def interpolate_lineout(xarr, yarr, **kwargs):
    ...
    plateaus = [
        list(g) for k, g in groupby(yarr)
    ]  # list of lists of identical values in a row
    ...
    interp_max = kwargs.pop("interpolate_max", 2000.0)

    # Find the central values of the plateaus
    for j, jj in enumerate(
        plateaus
    ):  # Working through the steps. jj is repeating list of yvals
        if xarr_consumable[0] < interp_max:  # Make sure we're below cutoff
            new_xvals.append(
                xarr_consumable[0 : len(jj)].mean()
            )  # Store center of the plateau 'bin'
            xarr_consumable = xarr_consumable[
                len(jj) :
            ]  # Trim off previous 'bin', prepare for the next 'bin'
        else:  # We're above threshold, stop
            # flatten the remaining list of lists into a list
            remaining_values = plateaus[j:]
            remaining_values = [
                item for sublist in remaining_values for item in sublist
            ]  # list comprehension magic
            # Append the remaining data point-by-point
            new_yvals = (
                new_yvals[:j] + remaining_values
            )  # Join the unique y values with those above threshold
...
def extract_line(data_set, field_name, axis, Nmax, interp=True, inc=1e-12, **kwargs):
    ...
    if data_set.geometry == "cylindrical":
        ...
        xmin = 0.0
        xmax = data_set.domain_width[0].d + xmin  # the max range of r
        zmin = data_set.index.grid_left_edge[0][1].d
        # the min value of z
        ...
        if axis.lower() == "r":
            ...
            x_coords = [
                xmin_arb + (0.5 + j) * (xmax_arb - xmin_arb) / Nmax for j in range(Nmax)
            ]
            y_coords = [
                zmin_arb + (0.5 + j) * (zmax_arb - zmin_arb) / Nmax for j in range(Nmax)
            ]
            z_coords = [np.pi for j in range(Nmax)]
    ...
    if data_set.geometry == "cartesian":
        ...
        xmin = data_set.index.grid_left_edge[0][0].d
        xmax = data_set.domain_width[0].d + xmin
        ymin = data_set.index.grid_left_edge[0][1].d
        ymax = data_set.domain_width[1].d + ymin
        zmin = data_set.index.grid_left_edge[0][2].d
        zmax = data_set.domain_width[2].d + zmin
        ...
        if axis.lower() == "x":
            x_coords = [
                xmin + (0.5 + j) * np.abs(xmax - xmin) / Nmax for j in range(Nmax)
            ]
            ...
            z_coords = [
                zmin + (0.5 + j) * np.abs(zmax - zmin) / Nmax for j in range(Nmax)
            ]
            xarr = np.array(y_coords)

        elif axis.lower() == "arb":
            x_coords = [
                xmin_arb + (0.5 + j) * np.abs(xmax_arb - xmin_arb) / Nmax
                for j in range(Nmax)
            ]
            y_coords = [
                ymin_arb + (0.5 + j) * np.abs(ymax_arb - ymin_arb) / Nmax
                for j in range(Nmax)
            ]
            z_coords = [
                zmin_arb + (0.5 + j) * np.abs(zmax_arb - zmin_arb) / Nmax
                for j in range(Nmax)
            ]
            xarr = np.sqrt(
                np.power(np.array(x_coords) - xmin_arb, 2)
    ...
    for j in range(Nmax):
        ...
        try:
            ...
            DataPoint = data_set.point(
                [x_coords[j] + inc, y_coords[j] + inc, z_coords[j] + inc]
            )
            yarr[j] = DataPoint["flash", field_name].in_cgs().d[0]
    ...
    if interp:
        ...
        if kwargs.pop("xyz", False):
            return (
                ...
                np.asarray(y_coords),
                np.asarray(z_coords),
                np.asarray(new_xvals),
                np.asarray(new_yvals),
            )
        else:
